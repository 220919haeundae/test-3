package com.kh.ch03_control;

import java.util.Scanner;

public class ControlRun {

	public static void main(String[] args) {

		/*
		 * * 프로그램의 진행은 순차적으로 진행됨(위에서 아래로, 왼쪽에서 오른쪽으로)
		 * 	이 흐름을 바꾸고자 할 때 제어문을 통해 직접 제어 가능
		 * 
		 * - 선택적으로 실행시키고자 할 때 => 조건문
		 * - 반복적으로 실행기키고자 할 때 => 반복문
		 * - 그 외 흐름 제어 => 분기문
		 */
		/*
		 * * 조건문 : 조건식을 통해 참/거짓에 따라 코드를 실행
		 *  - 종류
		 *   [1] if문
		 *   [2] switch문
		 */
		/*
		Scanner sc = new Scanner(System.in);
		
		System.out.print("숫자 입력 : ");
		int num = sc.nextInt();
		
		//입력된 값이 양수인지 양수가 아닌지 판별
		if(num > 0) {	//조건식의 결과가 true인 경우 if 중괄호 안의 코드 실행
			System.out.println(num+ "의 값은 양수다.");
			
		} else {		//조건식의 결과가 false인 경우 else 중괄호 안의 코드 실행
			System.out.println(num +"의 값은 양수가 아니다.");
			
		}
		*/
		
		Scanner sc = new Scanner(System.in);
		/**
		 * 사용자에게 구매할 과일을 입력받아 가격을 출력하기
		 *  - 수박 : 15000
		 *  - 참외 : 5000
		 *   - 복숭아 : 1000
		 *   
		 */
		/*
		System.out.print("구매할 과일을 입력(수박/참외/복숭아) : ");
		String fruit = sc.next();
		int price = 0;
		switch(fruit) {
		case "수박" :
			price = 15000;
			break;
		case "참외" :
			price = 5000;
			break;
		case "복숭아" :
			price = 10000;
			break;
		}
		System.out.printf("%s는 %d원입니다.", fruit, price);
		*/
		
//		int dan = (int)(Math.random() * (9-2) +2);
////		for(int i = 1; i <=9 ; i++) {
////			System.out.printf("%d X %d = %d\n", dan, i, dan*i);
////		}
		
		
		/*
		 * ToDo : 위의 랜덤값을 기준으로 아래와 같이 출력해보기
		 */
		
//		for(int i = 0; i < dan; i++) {
//			
//			for(int j = 0; j < dan; j++) {
//				if(i == j) {
//					System.out.print(i+1);
//				} else {
//					System.out.print("*");
//				}
//				
//			}
//			System.out.println();
//		}
		
		// ===========================
		/*
		 */
//		int dan = (int)(Math.random() * (9-2) +2);
//		int i = 1;
//		while(i < 10) {
//			System.out.printf("%d X %d = %d\n", dan, i, dan*i);
//			i++;
//		}
		
		// -------------------------------
		/*
		 * 분기문
		 * 	- continue : 코드를 실행하다가 이 분기문을 만나면 현재 회차를 넘기고 다음 분기로 이행
		 * 	- break	: 반복문을 강제로 빠져나감(종료)
		 * 		* switch문 안의 break는 switch문 만을 빠져나감
		 * 
		 * 
		 */
		// 1~10 사이의 홀수만 출력 ==>
//		for(int i =0; i < 10; i++) {
//			if(i%2==0) {
//				continue;
//			} 
//			System.out.print(i);
//		}
//		
		// 반복적으로 랜덤값(1 ~ 100)을 추출한 후 출력. 단 3의 배수인 경우 반복문 종료
		long startTime = System.nanoTime();
		while(true) {
			int r = (int)(Math.random()*100 +1);
			if(r % 3 ==0) {
				break;
			}
			System.out.println(r);
			
		}
		long endTime = System.nanoTime();
		System.out.println(endTime - startTime);
	}
}
